...
export default function UserMetaCard() {
  ...
  useEffect(() => {
    if (user && user.prefs) {
        setJobTitle(user.prefs.jobTitle || "");
        setLocation(user.prefs.location || "");
        setFacebook(user.prefs.facebook || "");
        setTwitter(user.prefs.twitter || "");
        setLinkedin(user.prefs.linkedin || "");
        setInstagram(user.prefs.instagram || "");
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          // Upload to 'avatars' bucket. Use unique ID.
          // Note: 'avatars' bucket must exist in Appwrite Console.
          ...
          const updatedPrefs = {
              ...user?.prefs, // keep existing fields
              avatar: fileUrl.href
          };
          ...
          alert(`Failed to upload image. Ensure 'avatars' bucket exists in Appwrite project: ${error.message}`);
      }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
        const updatedPrefs = {
            ...
            linkedin,
            instagram,
            avatar: avatarUrl // ensure avatar persists
        };
        await account.updatePrefs(updatedPrefs);
        await refreshUser();
        closeModal();
          ...
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            ...
            >
              <Image
                ...
                unoptimized // Use unoptimized to avoid loading issues with external Appwrite URLs
              />
